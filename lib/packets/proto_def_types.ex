defmodule McProtocol.Packets.ProtoDefTypes do
  ...
  def encode_slot(data), do: nil
  def decode_slot(data), do: nil

  def encode_position(data), do: nil
  def decode_position(data), do: nil
  ...
  def decode_nbt(data) do
    # FIXME FIXME FIXME
    {data, ret} = McProtocol.NBT.Read.read(data)
    {ret, data}
  end

  def encode_optional_nbt(data), do: McProtocol.NBT.Write.write(data, true)
  def decode_optional_nbt(data) do
    # FIXME FIXME FIXME
    {data, ret} = McProtocol.NBT.Write.read(data, true)
    {ret, data}
  end
