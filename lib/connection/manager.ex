defmodule McProtocol.Connection.Manager do
  ...
  def init({socket, direction, orch_module}) do
    ...
    {transitions, handler_state} = handler.enter(params, stash)
  ...
  def handle_cast({:packet, data}, state) do
    packet = McProtocol.Packet.In.construct(state.stash.direction,
                                            state.stash.mode, data)

    {transitions, handler_state} = state.handler.handle(
      packet, state.stash, state.handler_state)
